...
const ishtml = function(op) {
  ...
  iCnt = iCnt
    ...
    .replace(REG.HTML_PATH, (str, $1, $2, $3, $4, $5) => {
      ...
      if (
        ...
        if (op.publishPath.match(REG.IS_HTTP) || op.publishPath.match(REG.PATH_IS_ABSLURE)) {
          fixPath = fn.formatUrl(path.join(op.publishPath, rPath));
        } else {
          fixPath = fn.formatUrl(path.join(path.relative(op.publishPath, op.baseUrl), rPath));
        }
    ...
    .replace(REG.HTML_SCRIPT_TAG_INLINE, (str, $1, $2) => {
      let rPath;
      if ($2.match(REG.IS_HTTP) || $2.match(REG.PATH_IS_ABSLURE)) {
        if (fn.isSameHost(op.publishPath, $2)) {
          rPath = path.join(op.baseUrl, path.relative(op.publishPath, $2));
        } else {
        ...
        if (op.publishPath.match(REG.IS_HTTP) || op.publishPath.match(REG.PATH_IS_ABSLURE)) {
          rPath = path.join(op.baseUrl, $2);
        } else {
          rPath = path.join(op.publishPath, $2);
        }
      }
