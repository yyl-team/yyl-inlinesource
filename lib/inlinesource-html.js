...
const ishtml = function(op) {
  ...
  if (op.content) {
    ...
    if (!fs.existsSync(op.path)) {
      return Promise.reject(`yyl-inlinesource html run error, path is not exists: ${op.path}`);
    }
  ...
  iCnt = iCnt
    ...
    .replace(REG.SCRIPT_TAG, (str, $1) => {
      ...
      cnt = cnt.replace(REG.JS_INLINE, (str, $1, $2, $3) => {
        let fixPath = path.join(path.relative(iPath, path.dirname(rPath)), $2);
    ...
    .replace(REG.LINK_TAG, (str, $1) => {
      ...
      const replaceHandle = (str, $1, $2, $3) => {
        ...
        let fixPath = path.join(path.relative(iPath, path.dirname(rPath)), inPath);
        fixPath = fn.formatUrl(fixPath);
    ...
    .replace(REG.STYLE_CONTENT, (str, $1, $2, $3) => {
      ...
      const replaceHandle = (str, $1, $2, $3) => {
        ...
        if (inPath.match(REG.IMG_INLINE)) {
          const imgPath = fn.hideUrlTail(path.join(iPath, inPath));
    ...
    .replace(REG.IMG_TAG, (str, $1, $2, $3) => {
      const rPath = $2;
      const imgPath = fn.hideUrlTail(path.join(iPath, rPath));
